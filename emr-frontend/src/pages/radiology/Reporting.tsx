...
import { api, ApiError, can, openBlob } from '../../api/client';
...
function ReportEditor({ id, section, onClose }: { id: string; section: Section; onClose: () => void }) {
  ...
  const it = q.data;
  const isReporter = can(user, 'admin') || (section === 'radiology' ? can(user, 'radiologist') : can(user, 'endoscopist'));
  const canImages = can(user, 'admin', 'radiologist', 'radiographer', 'endoscopist', 'endoscopy_nurse');
  const endoStaff = can(user, 'admin', 'endoscopist', 'endoscopy_nurse');
